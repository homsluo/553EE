...
Bitmap::BitmapRead(const char* name){
    ifstream f(name);
    if(!f){
        cout << "open failed!"<< endl;
        return;
    }
    f >> fileHeader.bfType >> fileHeader.bfSize >> fileHeader.bfReserved1 >>
    fileHeader.bfReserved2 >> fileHeader.bfOffBits >> bitmapHeader.biSize
}
